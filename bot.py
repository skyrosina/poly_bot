...
Safety systems:
  ...
  3. Balance-verified buys: snapshot pUSD/collateral before/after; ghost fills
  ...
  5. Window-boundary balance sync: real pUSD/collateral balance overwrites internal
...
def check_polymarket_geoblock(timeout: int = 10) -> dict:
    """Return Polymarket's geoblock result for the current network route."""
...
class PolyBot:
    def __init__(self):
        ...
        self.executor = Executor(
            ...
            clob_api_passphrase=os.getenv("CLOB_PASS_PHRASE", ""),
    ...
    def start(self):
        if not self.dry_run and self.check_geoblock:
            try:
                geo = check_polymarket_geoblock()
                country = geo.get("country", "unknown")
                region = geo.get("region", "")
                blocked = bool(geo.get("blocked"))
                print(f"\n  Geoblock check: country={country} region={region or '-'} blocked={blocked}")
                if blocked:
                    print("  Live trading stopped: current IP is blocked by Polymarket geoblock.")
                    return
            except Exception as e:
                print(f"\n  Geoblock check failed: {e}")
                print("  Live trading stopped until geoblock status can be verified.")
                return

        if not self.dry_run and self.use_tor:
